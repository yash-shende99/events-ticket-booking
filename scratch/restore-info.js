...
async function main() {
  ...
  for (const title of RESTORE_MOVIES) {
    const htmlFile = htmlFiles.find(f => f.toLowerCase().startsWith(title.toLowerCase()));
    if (!htmlFile) {
      console.log(`❌ Could not find HTML file for ${title}`);
      continue;
    }

    const htmlPath = path.join(REF_DIR, htmlFile);
