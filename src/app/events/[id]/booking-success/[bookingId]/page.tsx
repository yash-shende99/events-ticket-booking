...
export default async function EventBookingSuccessPage({
  ...
  await connectDB();
  const ticket = await EventTicket.findOne({ bookingId: params.bookingId })
    .populate("event")
    .lean();
  ...
  const ticketSummary = ticket.tickets.map((t: any) => `${t.count}x ${t.tier}`).join(", ");
            ...
            <div className="space-y-4">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">{event?.category} • {event?.languages?.join(", ")}</p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="font-medium text-gray-800 text-sm">{event?.location}</h3>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="font-medium text-gray-800 text-sm">{event?.date} | 05:30 PM</h3>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-[#f84464] text-sm">Tickets: {ticketSummary}</h3>
                </div>
              </div>
            </div>
          </div>
